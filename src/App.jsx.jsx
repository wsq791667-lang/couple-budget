...
const SK_JW   = "snaps_jiwon";
const SK_SH   = "snaps_suhyun";
const FIXED_K = "fixed_exp";
const INST_K  = "installments";
const PROF_K  = "profiles";
const CHAT_K  = "chats";
...
function Sheet({onClose,title,children}){
  return(<div style={{position:"fixed",inset:0,background:"rgba(61,46,30,.45)",zIndex:200,display:"flex",alignItems:"flex-end"}} onClick={e=>e.target===e.currentTarget&&onClose()}><div style={{background:C.card,borderRadius:"22px 22px 0 0",padding:"24px 20px 44px",width:"100%",maxWidth:480,margin:"0 auto",maxHeight:"95vh",overflowY:"auto"}}>{title&&<div style={{fontSize:17,fontWeight:800,color:C.text,marginBottom:22}}>{title}</div>}{children}</div></div>);
}
function DelModal({onCancel,onConfirm}){
  return(<div style={{position:"fixed",inset:0,background:"rgba(61,46,30,.45)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{background:C.card,borderRadius:18,padding:24,width:280,textAlign:"center"}}><div style={{fontSize:15,fontWeight:700,color:C.text,marginBottom:6}}>삭제할까요?</div><div style={{fontSize:12,color:C.sub,marginBottom:20}}>삭제한 기록은 복구할 수 없어요</div><div style={{display:"flex",gap:8}}><button onClick={onCancel} style={{flex:1,padding:"10px 0",border:`1.5px solid ${C.border}`,borderRadius:10,background:C.card,color:C.sub,fontWeight:600,cursor:"pointer"}}>취소</button><button onClick={onConfirm} style={{flex:1,padding:"10px 0",border:"none",borderRadius:10,background:C.red,color:"#fff",fontWeight:700,cursor:"pointer"}}>삭제</button></div></div></div>);
}
...
function InputSheet({who,onClose,onSave,prevSnap}){
  ...
  return(
    <Sheet onClose={onClose} title={`${m.label} 입력`}>
      ...
      <button onClick={()=>{if(form.cash||form.card)onSave(form);}} style={bigBtn("#fff",`linear-gradient(135deg,${m.color},${m.color}cc)`)}>저장하기</button>
    </Sheet>
  );
}
...
function ChatTab({chats,onSend,profiles}){
  const [who,setWho]=useState("jiwon");
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 180px)"}}>
      ...
      <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,background:C.bg}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="메시지를 입력하세요..." style={{...inp,flex:1,padding:"10px 14px",fontSize:14}}/>
...
export default function App(){
  ...
  const load=useCallback(async()=>{
    ...
    const[jw,sh,fe,inst,prof,ch]=await Promise.all([
      fbGet(SK_JW,[]),fbGet(SK_SH,[]),fbGet(FIXED_K,[]),fbGet(INST_K,[]),fbGet(PROF_K,{jiwon:{},suhyun:{}}),fbGet(CHAT_K,[])
    ]);
  ...
  // 채팅 실시간 구독
  useEffect(()=>{
    const unsub=onSnapshot(doc(db,CHAT_K,"data"),(snap)=>{
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:72}}>
      ...
      <div style={{padding:"16px 16px 0"}}>
        ...
        :tab==="chat"   ?<ChatTab chats={chats} onSend={sendChat} profiles={profiles}/>
      ...
      {inputWho&&<InputSheet who={inputWho} onClose={()=>setInputWho(null)} onSave={(form)=>saveSnap(inputWho,form)} prevSnap={inputWho==="jiwon"?snapsJw[0]:snapsSh[0]}/>}
    </div>
  );
}
